...
const ZiWei = (function () {
  ...
  function renderIztro(astrolabe, gender, tst) {
    ...
    if (mingIdx >= 0) {
      ...
      // Auxiliary stars
      var auxTexts = {'文昌':'文昌入命，聪明好学，考试运佳。','文曲':'文曲入命，才艺出众，口才佳。','左辅':'左辅入命，贵人相助，人缘好。','右弼':'右弼入命，善于配合协作。','天魁':'天魁入命，白天贵人多。','天钺':'天钺入命，夜晚贵人多。','禄存':'禄存入命，财运稳健。','天马':'天马入命，多走动变迁。','擎羊':'擎羊入命，性格刚烈，魄力强。','陀罗':'陀罗入命，坚韧不拔但易拖延。','火星':'火星入命，行动力极强但冲动。','铃星':'铃星入命，内心焦虑但爆发力强。','地空':'地空入命，想法超前，适合哲学宗教。','地劫':'地劫入命，财来财去但悟性高。','红鸾':'红鸾入命，桃花旺盛。','天喜':'天喜入命，喜庆常伴。','华盖':'华盖入命，聪明孤高，有宗教缘。','天刑':'天刑入命，适合法律医疗。'};
      (ming.minorStars||[]).concat(ming.adjectiveStars||[]).forEach(function(s) { if (auxTexts[s.name]) html += '<p>' + auxTexts[s.name] + '</p>'; });
      html += '</div>';
    }
